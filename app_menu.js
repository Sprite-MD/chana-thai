...
const myFiles = [ "./menu/appetizers.json", "./menu/salad.json", "./menu/soup.json" ];
...
myFiles.forEach((file, i) => {
    ...
    loadFile(file, (responseText) => {
        ...
        food_data[i] = JSON.parse(responseText);
...
function display(){
    ...
    function insert_appetizers(){
        ...
        card_category.textContent = food_data[0].name;
        menu_card.appendChild(card_category);
    

        for (let i=0; i < food_data[0].items.length / 2; i++){
            ...
            food_name.textContent = food_data[0].items[i].name;
        
            const price = document.createElement('div');
            price.textContent = food_data[0].items[i].price;
        
            const food_description = document.createElement('p');
            food_description.textContent = food_data[0].items[i].description;
    ...
    function insert_salad(){
        ...
        for (let i=7; i < food_data[0].items.length; i++){
            const food_item = document.createElement('div');
            food_item.classList.add('food_item');
    ...
    app_btn.addEventListener('click', insert_salad);
}